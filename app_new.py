...
def mac():
    ...
    for row in jamf_rows:
        ...
        try:
            ...
            user_id = response.json().get('channel')
            email = row[1]  # assign email from the 'Name/Email' column
            row.append(channel)  # add the user ID to the row
        except:
            row.append('Message Sent')  # if the email is not found, add 'N/A'
    jamf_rows = [[row[0], row[1], row[2], ''] for row in jamf_rows]
